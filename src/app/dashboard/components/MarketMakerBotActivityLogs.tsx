...
interface MarketMakerBotActivityLogsProps {
  token: string;
}

export default function MarketMakerBotActivityLogs({ token }: MarketMakerBotActivityLogsProps) {
  const [logs, setLogs] = useState<MarketMakerBotLog[]>([]);
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      const response = await API.marketMakerBot.getLogs(100, token);
      if (response.code === '0' && response.data) {
        setLogs(response.data);
      }
    } catch (err) {
      console.error('Error fetching market maker bot logs:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && logs.length === 0) {
    return (
      <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] rounded-2xl p-6 shadow-2xl border border-[#27272a]">
        <h2 className="text-lg font-bold text-white mb-4">Market Maker Bot Activity Logs</h2>
    ...
    <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] rounded-2xl p-6 shadow-2xl border border-[#27272a]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Market Maker Bot Activity Logs</h2>
            <p className="text-xs text-gray-400">Real-time market making operations</p>
          </div>
        </div>
        <button
          onClick={fetchLogs}
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
        {filteredLogs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-xs mt-1">Start a market maker bot to see activity</p>
          </div>
        ) : (
