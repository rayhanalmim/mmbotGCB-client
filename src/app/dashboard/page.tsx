...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchBotStats = async () => {
      try {
        const response = await fetch('https://mmbotservergcb.vercel.app/api/bot/conditions', {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          }
  ...
  useEffect(() => {
    ...
    const checkCredentials = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://mmbotservergcb.vercel.app';
           ...
           <div className="flex justify-between items-center h-14">
             ...
             <div className="flex items-center gap-2">
               ...
               <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-[#1a1a1a] border border-purple-500/20 rounded-lg hover:border-purple-500/40 transition-colors">
                 <span className="text-[10px] font-bold text-purple-400 uppercase">GCB</span>
                 <span className="text-xs font-bold text-white">
                   {balanceLoading ? '...' : gcbFree.toFixed(2)}
                 </span>
               </div>
               ...
               <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-[#1a1a1a] border border-green-500/20 rounded-lg hover:border-green-500/40 transition-colors">
                 <span className="text-[10px] font-bold text-green-400 uppercase">USDT</span>
                 <span className="text-xs font-bold text-white">
